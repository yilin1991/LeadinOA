...
namespace Leadin.BLL
{
	...
	public partial class Supplier
	{
		...
		/// 获得数据列表
		/// </summary>
		public List<Leadin.Model.Supplier> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Leadin.Model.Supplier> DataTableToList(DataTable dt)
		{
			List<Leadin.Model.Supplier> modelList = new List<Leadin.Model.Supplier>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Leadin.Model.Supplier model;
        ...
        {
            StringBuilder strNumId = new StringBuilder("OAG");
            strNumId.Append(DateTime.Now.ToString("yyMMdd"));

            Model.Supplier model = GetModel(GetMaxId());
            if (model != null)
            {
